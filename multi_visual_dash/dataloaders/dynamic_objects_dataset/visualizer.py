...
class DynamicObjectsVisualizer:
    def __init__(self, dataset_dir: str, min_pc_points: int, max_dist2bbox: int, visualize_bboxes: bool = False):
        ...
        self.do_dataloader = DynamicObjectsDataLoader(dataset_dir)
        self.min_pc_points = min_pc_points
        self.max_dist2bbox = max_dist2bbox
        self.visualize_bboxes = visualize_bboxes

    def __call__(self):
        for scenes in self.do_dataloader():
            f_scenes = self.do_dataloader.filter_scenes(scenes, self.min_pc_points, self.max_dist2bbox)
            t_scenes = self.do_dataloader.transform_scenes(f_scenes)
            for scene_name, scene_data in t_scenes.items():
                for sample_id, sample_data in scene_data['samples'].items():
                    viz_massage = self.sample_data2plot_data(sample_id, sample_data)
                    yield viz_massage
